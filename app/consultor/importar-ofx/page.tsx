...
export default function ImportarOFXPage() {
  ...
  const handleSyncApi = async () => {
    if (!bancoSync) { toast.error('Selecione um banco para sincronizar.'); return; }
    setSyncingApi(true);
  ...
  const handleSaveRule = () => {
    if (!rulePattern || !ruleCategory) {
      toast.error('Preencha o termo da descrição e a categoria.');
      return;
    }
  ...
  const handleDeleteRule = async (id: string) => {
    if (!(await confirmAsync('Excluir esta regra de autoclassificação?'))) return;
  ...
  const handleReconcile = (t: OFXTransaction, manualId: string) => {
    ...
    try {
      ...
      s.delete(t.id);
      setSelected(s);
      toast.success('Transação OFX conciliada com o lançamento previsto com sucesso!');
    } catch (e) {
      toast.error((e as Error).message);
    }
  };
